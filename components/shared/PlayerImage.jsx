...
//       setCropping(true); // cropper khulega
//     }
//   };
...
//   // Proper crop function (zoom + crop area handle karega)
...
//   return (
//     <Card className="w-full max-w-sm mx-auto p-4 space-y-4 shadow-lg rounded-2xl">
...
const PlayerImage = ({ userId, onClose = () => {} }) => {
  ...
  useEffect(() => {
    if (success) {
      ...
      // ✅ Turant close kar do aur state reset
      onClose();
      dispatch(resetProfileImageState());
    }
  }, [success, onClose, file, dispatch]);
  ...
  return (
    <Card className="w-full bg-gray-800 max-w-sm mx-auto p-4 space-y-4 shadow-lg rounded-2xl">
      <CardContent className="space-y-4 flex flex-col items-center">
        ...
        {cropping && (
          <div className="fixed inset-0 bg-black bg-opacity-70 flex flex-col items-center justify-center z-50">
            <div className="relative w-[90vw] h-[60vh] rounded-xl overflow-hidden">
              <Cropper
                image={preview}
                crop={crop}
                zoom={zoom}
