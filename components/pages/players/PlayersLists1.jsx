...
// // If you use a different URL in production, change it here or make it env-driven.
...
//   const [allowedUsers, setAllowedUsers] = useState(subscription?.no_of_users);
//   const [isBoardOpen, setIsBoardOpen] = useState(false); // Chat toggle
...
//   return (
//     <div id="print-section" className="space-y-6 relative p-4">
//       <ToastContainer position="top-right" autoClose={3000} hideProgressBar />
...
//       {/* Edit Section (styled from version 2) */}
//       <div className="mb-4 flex items-center w-sm gap-3 text-white bg-gradient-to-r from-[#114153] to-blue-800 p-1 rounded-lg shadow-lg">
//         <div className="font-bold text-xl text-white flex-1 tracking-wide">✨ Edit Sections</div>
//         <select
//           id="groupSize"
//           value={groupSize}
//           onChange={(e) => handlePresetChange(Number(e.target.value))}
...
//                   )}
//                 </h2>
//                 {/* show available label for first section like before if you want */}
//                 {idx === 0 && <AvailableLabel />}
//               </div>
...
//                             animate={{ opacity: 1, y: 0 }}
//                             transition={{ duration: 0.4, ease: "easeOut" }}
//                             className="flex flex-col flex-1 min-w-0 space-y-1 items-center"
//                           >
//                             {/* Player Name */}
//                             <motion.div
//                               whileHover={{ scale: 1.05 }}
...
//       {/* ✅ Floating Chat Board (same behavior as first code) */}
...
//               socket={socket} // pass socket if MessageBoard expects it
//             />
//           </div>
//         )}
...
//                           // FIX: clear previous selection so second click always works
//                           dispatch(setSelectedPlayer(null));

//                           // small delay so UI can update and highlight doesn't flicker
//                           setTimeout(() => {
//                             dispatch(
//                               setSelectedPlayer({
//                                 ...player,
...
//             // small delay to avoid highlight flicker and ensure clean reset
//             setTimeout(() => {
//               dispatch(setSelectedPlayer(null));
//             }, 120);
...
//   useEffect(() => {
//     // clear selected player when ladder changes
//     dispatch(setSelectedPlayer(null));
//     setIsOpen(false);
//   }, [ladderId, dispatch]);
...
//                           // 1) Clear previous selection immediately
//                           dispatch(setSelectedPlayer(null));
//                           // 2) Set new selected player after a tiny delay with a forced unique field
//                           setTimeout(() => {
//                             dispatch(
//                               setSelectedPlayer({
//                                 ...player,
...
//                               })
//                             );
//                             // open modal after selection is set
//                             setIsOpen(true);
//                           }, 20);
//                         }}
//                         className={`relative flex flex-col gap-2 items-center rounded-sm px-4  shadow-md py-4  transition-all
...
//       {/* Render EditPlayer only when modal should be open and a player is selected.
//           Use key based on _forceUpdate to force remount when same player clicked repeatedly */}
//       {isOpen && selectedPlayer && (
//         <EditPlayer
//           key={selectedPlayer._forceUpdate || selectedPlayer.id}
//           open={true}
//           onClose={() => {
...
//             // refresh profile and leaderboard (same as before)
//             if (user?.id) dispatch(fetchUserProfile(user.id));
//             if (ladderId) {
//               dispatch(fetchLeaderboard({ ladder_id: ladderId }));
//               dispatch(fetchGradebars(ladderId));
...
const PlayersLists1 = () => {
  ...
  const playerList = players?.[ladderId]?.data || [];
  const [isOpen, setIsOpen] = useState(false); // local modal open state
  const [currentPlayerId, setCurrentPlayerId] = useState(null); // local immediate id for modal
  const [moveLoading, setMoveLoading] = useState(false);
  const [loadingPlayers, setLoadingPlayers] = useState(true);
  const [searchQuery, setSearchQuery] = useState("");
  const [isProDialogOpen, setIsProDialogOpen] = useState(false);
  ...
  // ---------- Helper: open edit modal reliably ----------
  const openEditForPlayer = (player) => {
    // Quick safety checks
    const canEdit =
      user?.user_type?.toLowerCase() === "admin" || user?.id === player.user_id;
    const globalIndex = 0; // will be computed by caller if needed; we check allowed externally
    ...
    // 1) set local id immediately so modal opens
    setCurrentPlayerId(player.id);
    setIsOpen(true);

    // 2) force redux selection (clear then set) so other parts of app still get selectedPlayer
    dispatch(setSelectedPlayer(null));
    setTimeout(() => {
      dispatch(
        setSelectedPlayer({
          ...player,
          ladder_id: ladderId,
          // force unique object to ensure reducers that compare by reference see a change
          _forceUpdate: Date.now(),
        })
      );
    }, 20);
  ...
  return (
    <div id="print-section" className="space-y-6 relative p-4">
      ...
      {loadingPlayers ? (
        ...
        <div className="space-y-8">
          {grades.map((grade, idx) => (
            <div key={`${grade.gradebarId}-${idx}`} className="space-y-2">
              <div className="flex items-center gap-4">
                <h2 className="flex items-center gap-2 font-bold text-white border border-gray-400 px-4 py-1 rounded shadow-sm uppercase ">
                  {editingGradeId === grade.gradebarId ? (
                    <>
                      <input
                        value={tempGradeName}
              ...
              {/* --------------------
                  Responsive players list
                  Mobile-first: compact single-row list (image left, text right)
                  md+ : switch to card layout (image above, text centered)
                  -------------------- */}
              <div className="grid gap-4 mt-8 grid-cols-1 md:grid-cols-3 lg:grid-cols-4">
                {grade.players.map((player, pidx) => {
                  const isActive = selectedPlayer?.id === player.id;
                  const canEdit =
                    user?.user_type.toLowerCase() === "admin" ||
                  ...
                  // Mobile-first: row style. At md+ becomes column (card)
                  const itemBaseClasses = `
                    ...
                    items-center
                    gap-4
                    ${player.player_status === 1 ? "bg-green-700" : isActive ? "bg-yellow-400" : "bg-red-700 dark:bg-gray-800"}
                    ${isAllowed && canEdit ? "cursor-pointer hover:scale-[1.01]" : "cursor-not-allowed opacity-70"}
                    md:flex-col md:items-center md:py-4 md:px-4
                  `;
                  ...
                  return (
                    <motion.div
                      ...
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ duration: 0.25, delay: pidx * 0.02 }}
                    >
                      <div
                        onClick={() => {
                          if (!isAllowed) {
                          ...
                          // open modal and set current id immediately (fixes needing refresh)
                          openEditForPlayer(player);
                        }}
                        className={itemBaseClasses}
                      >
                        {/* Player image: smaller on mobile, larger on md+ */}
                        <div className="flex-shrink-0">
                          <Image
                            src={playerImageUrl}
                            className="rounded-full w-12 h-12 md:w-20 md:h-20 object-cover border border-zinc-300"
                            width={64}
                            height={64}
                            alt={`Player ${player.name}`}
                          />
                        ...
                        {/* Rank badge - absolute on md+ (top-left), on mobile small left badge */}
                        <div className="relative flex-1 w-full">
                          {/* Mobile: show rank inline at left; md+: absolute in top-left of card */}
                          <div className="flex items-center justify-start md:justify-center gap-2 w-full">
                            <div className="text-sm md:absolute md:top-1 md:left-2 md:text-2xl font-extrabold text-white drop-shadow-md">
                              {player.rank}
                            </div>

                            {/* Primary content */}
                            <div className="flex flex-col md:items-center md:text-center pl-0 md:pl-0">
                              <div className="flex items-center gap-2 md:flex-col">
                                <User className="text-indigo-500 w-5 h-5" />
                                <p
                                  className="text-base truncate font-semibold bg-gradient-to-r from-indigo-300 via-purple-100 px-2 md:px-6 rounded-sm"
                                  title={player.name}
                                >
                                  {player.name}
                                </p>
                              ...
                              <div className="flex items-center gap-2 text-indigo-200 text-sm mt-1 md:mt-2">
                                <Phone className="w-4 h-4" />
                                <p className="truncate">{player.phone}</p>
                              </div>
                            </div>
                          </div>
                        </div>
      ...
      {/* Render EditPlayer only when modal should be open.
          We pass currentPlayerId so modal can render immediately.
          We still synchronize Redux selectedPlayer for other logic. */}
      {isOpen && currentPlayerId && (
        <EditPlayer
          key={currentPlayerId + (selectedPlayer?._forceUpdate || "")}
          open={true}
          onClose={() => {
...
//   const [isOpen, setIsOpen] = useState(false); // local modal open state
//   const [currentPlayerId, setCurrentPlayerId] = useState(null); // local immediate id for modal
//   const [modalKey, setModalKey] = useState(0); // force remount key for modal
//   const [moveLoading, setMoveLoading] = useState(false);
//   const [loadingPlayers, setLoadingPlayers] = useState(true);
//   const [searchQuery, setSearchQuery] = useState("");
//   const [isProDialogOpen, setIsProDialogOpen] = useState(false);
...
//     // Clear previous selection/ modal first (ensures remount)
//     dispatch(setSelectedPlayer(null));
//     setIsOpen(false);
//     setCurrentPlayerId(null);

//     // small timeout to avoid race conditions and force fresh mount
//     setTimeout(() => {
//       // set local id (used by modal)
//       setCurrentPlayerId(player.id);

//       // set selected player in redux with a forced unique field so reducers detect change
//       dispatch(
//         setSelectedPlayer({
//           ...player,
//           ladder_id: ladderId,
...
//       // update modalKey to force EditPlayer remount every open
...
//       {isOpen && currentPlayerId && (
//         <EditPlayer
//           key={`${modalKey}-${currentPlayerId}`}
//           open={true}
//           onClose={() => {
//             // close modal and clear local + redux selection
//             setIsOpen(false);
//             setCurrentPlayerId(null);
//             dispatch(setSelectedPlayer(null));
...
//   const [isOpen, setIsOpen] = useState(false); // local modal open state
//   const [currentPlayerId, setCurrentPlayerId] = useState(null); // local immediate id for modal
//   const [openToken, setOpenToken] = useState(0); // increment only when opening
//   const [moveLoading, setMoveLoading] = useState(false);
//   const [loadingPlayers, setLoadingPlayers] = useState(true);
//   const [searchQuery, setSearchQuery] = useState("");
//   const [isProDialogOpen, setIsProDialogOpen] = useState(false);
...
//     // 1) Clear redux selected player and local modal state
//     dispatch(setSelectedPlayer(null));
//     setIsOpen(false);
//     setCurrentPlayerId(null);
...
//       // increment openToken exactly once per open action
//       setOpenToken((t) => t + 1);
...
//       {isOpen && currentPlayerId && (
//         <EditPlayer
//           // key ensures remount only when playerId or openToken changes
//           key={`modal-${currentPlayerId}-${openToken}`}
//           open={isOpen}
//           onClose={closeModal}
//           currentId={currentPlayerId}
//           setLoading={setMoveLoading}
...
//   const [isOpen, setIsOpen] = useState(false); // local modal open state
//   const [currentPlayerId, setCurrentPlayerId] = useState(null); // local immediate id for modal
//   const [openToken, setOpenToken] = useState(0); // increment only when intentionally opening
//   const [moveLoading, setMoveLoading] = useState(false);
//   const [loadingPlayers, setLoadingPlayers] = useState(true);
//   const [searchQuery, setSearchQuery] = useState("");
//   const [isProDialogOpen, setIsProDialogOpen] = useState(false);
...
//   // ---------- Helper: open edit modal reliably (final) ----------
//   const openEditForPlayer = (player) => {
//     const canEdit =
//       user?.user_type?.toLowerCase() === "admin" || user?.id === player.user_id;
...
//     // 1) Clear redux selected player + local modal immediately
//     setIsOpen(false);
//     setCurrentPlayerId(null);
//     dispatch(setSelectedPlayer(null));

//     // 2) WAIT briefly to allow cleanup and redux propagation, then set fresh values
//     setTimeout(() => {
//       setCurrentPlayerId(player.id);
...
//       // increment openToken exactly once per open action (stable)
//       setOpenToken((t) => t + 1);

//       // 3) finally open modal
//       setIsOpen(true);
//     }, 150); // 150ms gives React + Redux time to flush previous state
//   };
...
//         currentPlayerId &&
//         selectedPlayer &&
//         Number(selectedPlayer.id) === Number(currentPlayerId) && (
//           <EditPlayer
//             // key ensures remount only when playerId or openToken changes
//             key={`modal-${currentPlayerId}-${openToken}`}
//             open={isOpen}
...
//   // ---------- Helper: open edit modal reliably ----------
//   const openEditForPlayer = async (player) => {
//     const canEdit =
//       user?.user_type?.toLowerCase() === "admin" || user?.id === player.user_id;
...
//     try {
//       // 1) close any existing modal & clear redux selection first (immediate)
//       setIsOpen(false);
//       setCurrentPlayerId(null);
//       dispatch(setSelectedPlayer(null));

//       // 2) fetch latest leaderboard for this ladder (ensures players list is fresh)
//       if (ladderId) {
//         await dispatch(fetchLeaderboard({ ladder_id: ladderId }));
//       }

//       // 3) set selected player in redux (with force flag) so other parts see change
//       dispatch(
//         setSelectedPlayer({
//           ...player,
//           ladder_id: ladderId,
...
//       // 4) update key and open modal
//       setModalKey(Date.now());
//       setCurrentPlayerId(player.id);
//       setIsOpen(true);
//     } catch (err) {
//       console.error("openEditForPlayer error:", err);
//       // still try to open gently if fetch failed
//       dispatch(
//         setSelectedPlayer({
//           ...player,
//           ladder_id: ladderId,
