...
//             <AlertDialogDescription>
//               Higher-ranked players cannot challenge to lower-ranked players.
//             </AlertDialogDescription>
//           </AlertDialogHeader>
//           <AlertDialogFooter>
...
const MoveNumberInput = ({
  ...
  // Helpers for Best-of preview rendering
  const parseScorePair = (sc) => {
    // expects "3-1" etc.
    if (!sc || typeof sc !== "string") return [0, 0];
    const parts = sc.split("-").map((s) => Number(s));
    if (parts.length !== 2 || parts.some(isNaN)) return [0, 0];
    return parts;
  };

  const renderBo5Preview = () => {
    // Show two rows of 5 boxes: top = reporter (selectedPlayer), bottom = opponent (challengedPlayer)
    const [a, b] = parseScorePair(score); // e.g. [3,1]
    // Determine reporterWins & opponentWins depending on resultType
    let reporterWins = 0;
    let opponentWins = 0;
    if (score) {
      if (resultType === "beat") {
        ...
        // reporter lost -> opponent's score is first part in input (we still use a-b format)
        reporterWins = b;
        opponentWins = a;
      }
    }

    const box = (filled, idx) => (
      <div
        key={idx}
        className={`w-6 h-6 flex items-center justify-center rounded-sm border border-white text-[12px] ${
          filled ? "bg-white text-black font-bold" : "bg-transparent text-white/80"
        }`}
      >
        {filled ? "●" : ""}
      </div>
    ...
    return (
      <div className="mb-4 p-3 rounded-lg border border-gray-600 bg-gradient-to-r from-slate-900 to-slate-800">
        <div className="text-sm font-semibold text-gray-200 mb-2">Best of 5 Preview</div>
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex gap-1 mt-2">{Array.from({ length: 5 }).map((_, i) => box(i < reporterWins, `r-${i}`))}</div>
          </div>

          {/* Versus / rank */}
          <div className="flex flex-col items-center justify-center px-2">
            <div className="text-[10px] text-gray-400">vs</div>
            <div className="text-xs text-gray-200 font-semibold">#{selectedNumber || "--"}</div>
          </div>

          {/* Opponent row */}
          <div className="flex-1 min-w-0 text-right">
            <div className="text-xs text-gray-300 truncate">{challengedPlayer?.name || "Opponent"}</div>
            <div className="flex gap-1 justify-end mt-2">{Array.from({ length: 5 }).map((_, i) => box(i < opponentWins, `o-${i}`))}</div>
          </div>
        </div>

        <div className="mt-2 text-[11px] text-gray-400">
          Selected score: <span className="font-medium text-gray-200">{score || "—"}</span>
        </div>
      </div>
    );
  };
  ...
  return (
    ...
    >
      ...
      {/* BEST OF 5 PREVIEW - only when ladderType === "best5" */}
      {ladderType === "best5" && renderBo5Preview()}
